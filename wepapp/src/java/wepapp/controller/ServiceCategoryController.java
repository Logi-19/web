...
import wepapp.service.ServiceCategoryService;
...
// Front Controller Pattern - Single entry point for all service category-related requests
@WebServlet("/servicecategory/*")
public class ServiceCategoryController extends HttpServlet {
    ...
    private boolean isPageRequest(String pathInfo) {
        return pathInfo == null || 
               ...
               pathInfo.equals("/servicecategory.jsp");
    }
    
    // Handle page requests
    private void handlePageRequest(HttpServletRequest request, HttpServletResponse response, String pathInfo) 
            throws ServletException, IOException {
        if (pathInfo == null || pathInfo.equals("/") || pathInfo.equals("/servicecategory.jsp")) {
            // Show service category management page
            request.getRequestDispatcher("/servicecategory.jsp").forward(request, response);
        } else if (pathInfo.equals("/list")) {
            // Redirect to main page
            response.sendRedirect(request.getContextPath() + "/servicecategory/");
        } else {
    ...
    private void handleApiGetRequest(HttpServletRequest request, HttpServletResponse response, String pathInfo) 
        ...
        try {
            ...
            // Get recent categories
                ...
                List<ServiceCategory> categories = categoryService.getRecentCategories(limit);
                out.print(gson.toJson(categories));
            ...
            // Search categories
                ...
                if (keyword != null && !keyword.isEmpty()) {
                    List<ServiceCategory> categories = categoryService.searchCategories(keyword);
                    out.print(gson.toJson(categories));
                } else {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
    ...
    private void handleApiPostRequest(HttpServletRequest request, HttpServletResponse response, String pathInfo) 
        ...
        try {
            ...
            if (pathInfo.equals("/api/create")) {
                ServiceCategory category = extractCategoryFromRequest(request);
                ServiceCategoryService.ValidationResult result = categoryService.createCategory(category);
    ...
    private void handleApiPutRequest(HttpServletRequest request, HttpServletResponse response, String pathInfo) 
        ...
        try {
            ...
            if (pathInfo.matches("/api/\\d+")) {
                ...
                ServiceCategoryService.ValidationResult result = categoryService.updateCategory(category);
    ...
    // Helper method to extract ServiceCategory from request (JSON or form)
    private ServiceCategory extractCategoryFromRequest(HttpServletRequest request) throws IOException {
    ...
    // Extract ServiceCategory from JSON
    private ServiceCategory extractCategoryFromJson(HttpServletRequest request) throws IOException {
    ...
    // Extract ServiceCategory from form data
    private ServiceCategory extractCategoryFromForm(HttpServletRequest request) {
        ServiceCategory category = new ServiceCategory();
