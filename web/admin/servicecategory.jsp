...
<head>
    ...
    <title>Ocean View Resort · Manage Service Categories</title>
    ...
    <style>
        * { font-family: 'Inter', system-ui, sans-serif; }
        ...
        /* Service category badge styling with different colors */
        .category-badge-spa {
...
<body class="bg-[#f0f7fa] text-[#1e3c5c] antialiased flex">
    ...
    <main class="flex-1 overflow-y-auto" x-data="serviceCategoryManager()" x-init="init()">
        <!-- top bar -->
        <header class="bg-white/70 backdrop-blur-sm border-b border-[#b5e5e0] py-4 px-8 flex justify-between items-center sticky top-0 z-10">
            <div class="flex items-center gap-2 text-[#1e3c5c]">
                ...
                <span class="font-semibold">Service Category Management</span>
            </div>
        ...
        <div class="p-4 md:p-6 lg:p-8 space-y-6 max-w-full">
            ...
            <div class="flex justify-between items-center">
                <div>
                    <h2 class="text-2xl md:text-3xl font-bold text-[#1e3c5c]">Manage Service Categories 💆‍♀️</h2>
                    <p class="text-[#3a5a78] text-base mt-1">Add, edit and manage spa & wellness service categories</p>
                </div>
                                ...
                                <tbody>
                                    <template x-for="category in paginatedCategories" :key="category.id">
                                        <tr>
                                            ...
                                            <td>
                                                <span :class="getCategoryBadgeClass(category.name)">
                ...
                <form @submit.prevent="saveCategory()" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" 
                               ...
                               required
                               placeholder="e.g., Spa & Wellness, Massage Therapy, Ayurveda, Beauty Treatments"
                        ...
                        <p class="text-xs text-[#3a5a78] mt-1">Enter a unique service category name</p>
                    </div>
    ...
    <script>
        function serviceCategoryManager() {
            return {
                ...
                // Helper method to get appropriate badge class based on category name
                getCategoryBadgeClass: function(categoryName) {
                    var name = categoryName.toLowerCase();
                    if (name.includes('spa') || name.includes('wellness')) return 'category-badge-spa';
                    if (name.includes('massage')) return 'category-badge-massage';
                    if (name.includes('ayurveda')) return 'category-badge-ayurveda';
                    if (name.includes('beauty') || name.includes('facial') || name.includes('body')) return 'category-badge-beauty';
                    if (name.includes('yoga') || name.includes('meditation')) return 'category-badge-yoga';
                    if (name.includes('fitness') || name.includes('hydro')) return 'category-badge-fitness';
                ...
                searchCategories: function() {
                    ...
                    var self = this;
                    fetch('${pageContext.request.contextPath}/servicecategory/api/search?q=' + encodeURIComponent(this.searchQuery))
                        .then(response => response.json())
                        .then(data => {
                            self.categories = data;
                        })
                ...
                checkNameExists: function() {
                    ...
                    var self = this;
                    var url = '${pageContext.request.contextPath}/servicecategory/api/exists?name=' + encodeURIComponent(this.formData.name);
                ...
                saveCategory: function() {
                    ...
                    var url = this.formMode === 'new' 
                        ? '${pageContext.request.contextPath}/servicecategory/api/create'
                        : '${pageContext.request.contextPath}/servicecategory/api/' + this.formData.id;
                ...
                deleteCategory: function() {
                    ...
                    fetch('${pageContext.request.contextPath}/servicecategory/api/' + this.selectedCategory.id, {
                        method: 'DELETE'
                    })
                    .then(response => response.json())
                    .then(data => {
